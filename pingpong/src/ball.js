...
export default class Ball {
    ...
    get x() {
        return parseFloat(getComputedStyle(this.gameBall).getPropertyValue("--positionX"));
    }
    set x(value) {
        this.gameBall.style.setProperty("--positionX", value);
    }
    get y() {
        return parseFloat(getComputedStyle(this.gameBall).getPropertyValue("--positionY"));
    }
    set y(value) {
    ...
    reset() {
        ...
        this.y = 50;
        this.ballDirection = { x: 0, y: 0 };
        while (Math.abs(this.ballDirection.x) <= 0.2 ||
